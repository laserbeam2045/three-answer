...
export default function CardTile({
  ...
  const cls = [
    "card-tile relative inline-flex items-center justify-center font-bold tracking-wide whitespace-nowrap",
    SIZE_CLS[size],
    state === "selected" ? "selected" : "",
    state === "revealed" ? "outline outline-2 outline-gold/70" : "",
  ...
  const bar = role ? (
    <span
      aria-hidden
      className={`absolute inset-x-1.5 bottom-1 h-1 rounded-full ${ROLE_BAR[role]}`}
    />
  ) : null;
