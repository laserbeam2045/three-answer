...
### デザイン言語「金縁の舞台」
- ダークテーマ固定。濃紺の舞台（`#0a0d17`、七宝つなぎの地紋 + 上からのスポットライト）に **金の装飾枠**。
  アクセントはプレイヤー色 A=`#ec4899`(pink) / B=`#3b82f6`(blue) / C=`#22c55e`(green)、正解=金 `#e2c05b`、不正解=紅 `#e14b4b`。
- **装飾は SVG で描く**（拡大しても崩れず、色を揃えやすい）。`public/ornaments/frame.svg` を `border-image` で四隅（菱形と扇）と辺（連珠）に分割した額縁 `.ornate` / `.ornate-2`、
  トップページのエンブレム `emblem.svg`、地紋・和紙のノイズは CSS 内の data URI。見出しは飾り罫つき `OrnateTitle`（◆ と両側の金線）。
- **札**は和紙質感（feTurbulence のノイズ）に二重の縁取り、下端にプレイヤー色の帯。裏向きの札は紺地に金の七宝。
- フォント: 本文は Noto Sans JP（`next/font/google`）、**見出し・札・答え・演出は明朝体 Shippori Mincho B1**（Google Fonts から `<link>` で読み込む。
  `next/font` で束ねると日本語フォントのスライス 240 件超をビルド時に取得する必要があり、ネットワーク事情でビルドが落ちるため）。
- 部品: 金のボタン `.btn-gold`（グラデーションと段差の影）、縁取りボタン `.btn-ghost`、銘板 `.plaque`（第n問）、目盛りつきゲージ `.gauge`、判定の印 `.seal-ok/.seal-ng`、結果のメダリオン `.medallion`（背後に光条 `.rays`）。
- レスポンシブ必須（スマホ縦でプレイ可能に。3席・3スロットは横並び）。額縁は `border-image` なので角丸は使わない。
- 演出はCSS transitionで軽く（フリップ、フェード、正解時の金色グローと紙吹雪）。効果音なし。
...
- **構成（必須）**: Q1〜Q5 は単独1枚、**Q6〜Q20 は全て2枚以上の複合**。3枚合体は Q20 の決め所だけでなく **Q10 以降に複数置く**（目標5問・最低3問。3人全員で合わせる場面を増やすため）。3枚合体は全員が必要で罠を置く席が無いので罠は不要。
...
- **難易度のグラデーション（必須）**: quizzes3 の正答率（corrected_num / answered_num）で測る。
  - **各セット内で Q1 → Q20 に向けて難しくする**。Q1 が最も易しく、Q19 に向けて正答率が下がる
    （Q20 は3枚合体の決め所なので知識難易度の順序からは除外）。目安 **Q1≈0.70 → Q19≈0.50**（下限 0.46）。
    0.9台は簡単すぎ、0.45以下は3人の連携と合わさると終盤が難しすぎるため両端を切る。
...
- **判定の公平性（必須・機械検証）**: どのプレイヤーも「自分の手札と問題文だけ」で出すか決められること。
  出しても出さなくても筋が通る状況を作ってはいけない（＝理不尽な判定）。ユーザーから繰り返し指摘された最重要の不備。
  1. 自分の札が正解の読みの**中に**、自分の席順と矛盾しない位置で現れてはいけない
     （✗ 正解「しんぞう」で C が「ぞう」を持つ。C は「誰かが しん を持つのでは」と考えうる）。
     同じ席が「しょうか」と「しょう」のように、正解の読みの先頭を共有する2枚を持つのも不可。
  2. **答えの表記が読みより長い問題**（「何体？」→ 染色体 / せんしょく、「何銀行？」→ 日本銀行 / にっぽん）では、
     **省かれた部分（たい・ぎんこう）の札を、席順の矛盾しない席が持ってはいけない**
     （✗ C が「たい」を持つ。「染色体なら たい も要るのでは」と考えうるのに、出すと不正解）。
     これを「罠」として置くのは禁止。`tools/check-extension.mjs`（`npm run validate-sets` に含む）が
     分類語彙表で表記の完全な読みを引いて検出する。辞書に無い語は「判定不可」として目視で確認する。
  `tools/skeleton.mjs` は骨格の時点で 1・2 の両方を満たす札しか配らない。
