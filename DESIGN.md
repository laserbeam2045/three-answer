...
- 各カードの役割を設計する:
  ...
  - 引っかけ: 正解には不要だが「出すべきか？」と悩む札を意図的に配る（例: サンドイッチ問題に れたす・はむ、駅伝問題に かなくりしそうはい、都道府県問題に別の近しい県）。引っかけは熟考すれば「出さない」と判断できる明確な根拠があること。
  - **緊張保証（必須）**: 全20問について、正解に不要なプレイヤーのうち**少なくとも1人**（可能なら全員）に「出すか悩める札」があること。緊張マトリクス（Q×A/B/C、R=必要/T=罠/−=無関心）で「−が2つ並ぶ問題」はゼロにする。また各プレイヤーに3問以上の連続無関心区間を作らない。1枚の罠札が複数の問題をカバーする多重役割が理想。
