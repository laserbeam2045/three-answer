...
- **難易度のグラデーション（必須）**: quizzes3 の正答率（corrected_num / answered_num）で測る。
  - セット内: Q1が最も易しく、Q19に向けて正答率が下がる（Q20は3枚合体の決め所なので順序から除外）。
  - セット間: set1 → set5 で平均正答率が下がる。目安 set1≈0.75 / set2≈0.60 / set3≈0.50 / set4≈0.42 / set5≈0.33。
  - `npm run check-sources` が各問の正答率・平均・逆転箇所・セット間の逆転を報告する。
